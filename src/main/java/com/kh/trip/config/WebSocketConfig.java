...
public class WebSocketConfig implements WebSocketMessageBrokerConfigurer {
    ...
    public void configureMessageBroker(MessageBrokerRegistry config) {
        // 서버에서 클라이언트로 메시지를 보낼 때 사용할 prefix 설정
        // 클라이언트가 "/topic/..." 경로를 구독(subscribe)하고 있으면 메시지를 받을 수 있도록 설정
        config.enableSimpleBroker("/topic");

        // 클라이언트가 서버로 메시지를 보낼 때(발행) 사용할 prefix 설정
        //리액트에서 메시지를 보낼 때 주소 앞에 "/app"을 붙여야 @MessageMapping 컨트롤러로 전달, "/app/chat"으로 보내면 서버의 @MessageMapping("/chat")이 실행
    ...
    public void configureClientInboundChannel(ChannelRegistration registration) {
    	// STOMP CONNECT/SEND/SUBSCRIBE 시점에 인증 요청을 가로채 인증 정보를 설정
